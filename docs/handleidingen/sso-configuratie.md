...
3. Vul in:
   ...
   - **Omleidings-URI**: `https://jouw-domein.nl/oauth/oidc/callback`
...
Redirect URI: `https://jouw-domein.nl/oauth/google/callback`

### Generieke OIDC provider

Elke OIDC-compatibele provider (Keycloak, Okta, Authentik) werkt met dezelfde variabelen:

```bash
OPENID_PROVIDER_URL=https://jouw-provider/.well-known/openid-configuration
...
Redirect URI: `https://jouw-domein.nl/oauth/oidc/callback`

:::info Beperking
Er kan slechts **één** OIDC provider tegelijk worden geconfigureerd via omgevingsvariabelen. Google OAuth is de uitzondering (heeft eigen variabelen).
:::

### Trusted Header authenticatie (reverse proxy)

Voor deployments achter een authenticerende reverse proxy (Pomerium, oauth2-proxy, Authelia):
